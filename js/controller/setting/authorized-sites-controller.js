...
export class AuthorizedSitesController {
  constructor({ wallet }) {
    this.wallet = wallet;
    this.cachedSites = [];
  ...
  async loadAuthorizedSites() {
    try {
      ...
      console.error('[AuthorizedSitesController] 加载授权网站失败:', error);
      this.cachedSites = [];
  ...
  async handleRevokeSite(origin, options = {}) {
    ...
    try {
      ...
      console.error('[AuthorizedSitesController] 撤销授权失败:', error);
  ...
  async handleClearAllAuthorizations() {
    ...
    try {
      ...
      console.error('[AuthorizedSitesController] 清除所有授权失败:', error);
  ...
  async loadSiteUcanSession(origin, address) {
    try {
      ...
      console.error('[AuthorizedSitesController] 获取 UCAN 会话失败:', error);
      if (!this.activeSiteDetail || this.activeSiteDetail.origin !== origin || this.activeSiteDetail.address !== address) {
        return;
      }
