...
> 状态：提案
>
> 适用版本：跨系统目标方案；Wallet `1.4.22` 已完成社区身份绑定与邮箱验证基础流程，Passport-backed Wallet Login 仍需 Node、Wallet、第三方应用协同实现
>
> 目标读者：Wallet、Node Identity、Chat、Project 和安全架构维护者
>
> 目标：让社区应用在“钱包插件登录”和“无钱包通行证登录”两种入口下都收敛到同一钱包身份，并在用户授权后获得身份验证服务签发的已验证用户名和邮箱声明。当前 `subjectId` / `sub_xxx` 仅是 Node 的兼容索引，不是钱包身份的归属或长期对外主键。

> 所有权边界：钱包身份、地址控制证明和用户持有的凭证属于用户；Node Passport 是当前可替换的验证与声明签发服务。它在邮箱验证期间处理邮箱，但不拥有用户身份、钱包资产或业务资料。

> 验证语义：用户名由钱包控制权认证后向验证服务登记，服务执行规范化和唯一性校验并签发用户名声明；邮箱通过验证码验证控制权。两者只有在 DApp 显式请求相应 scope 且用户确认后才会出现在 assertion 中。
>
...
用户首次以 Wallet 完成地址控制证明并创建社区身份；绑定社区邮箱并完成验证码后，邮箱成为 Passport subject 上的已验证声明。以后用户可任选 Wallet 或 Passkey 登录。两种路径都必须由 Node 映射到相同的 `subjectId`、已授权声明、组织成员关系和业务权限。

本方案只处理“认证，即谁在登录”。登录后的细粒度服务授权仍使用 UCAN 或服务侧的短期 scoped credential：

- SIWE：证明控制某一 Ethereum 地址。
- Passkey：证明控制已绑定通行证的设备/系统凭据。
- Node session / Passport assertion：向社区应用表达已认证的 `subjectId`、认证方式和被授权 scope 下的声明。
...
| Wallet | 地址控制证明、签名、用户侧高风险确认、向 Node 领取 Passport assertion | 首次绑定、改绑/恢复的 SIWE 或明确签名；在 DApp 请求时把钱包证明与 Node 背书声明一并返回 | 代替 Node 校验 WebAuthn，保存 passkey 私钥，或自行声明邮箱已验证 |
| Chat / Project / Agent 等 Relying App | 自己的业务状态和权限 | 验证 Node session / Passport assertion，按 `subjectId` 做业务授权 | 自行创建 credential、相信浏览器传来的地址，或信任钱包签名中的自声明邮箱 |
...
```json
{
  ...
  "emailVerifiedAt": "2026-08-01T12:05:00Z",
  "walletBindings": [{
    "address": "0x1234...",
    "chainId": 1,
...
### 4.3 Passport-backed Wallet Login（按需升级的钱包登录路径）

社区 Web3 应用通常已有“钱包登录”入口；不需要社区身份或邮箱时，原有 SIWE 钱包登录应继续可用。只有当 DApp 需要 `subjectId`、已验证邮箱或跨“无钱包通行证登录”统一身份时，才把钱包登录升级为 **Passport-backed Wallet Login**：Wallet 向 Node 获取 Passport assertion，一并返回给 DApp。钱包签名只证明地址控制权，`subjectId`、已验证邮箱和授权 scope 由 Node 签发的 assertion 背书。

第三方应用应优先复用已有 SIWE 登录接口，不强制新增平行登录路径。升级点不是新增 `mode`，而是扩展 SIWE challenge 的 `scope`：

- `POST /api/v1/public/auth/challenge`：增加 `scope` 字段。未传 `scope` 或只请求钱包地址时，继续返回普通 SIWE challenge；请求 Passport identity scope 时，额外返回 Wallet 请求 assertion 所需的 `appId`、`audience`、`scope`、`passportEndpoint`。
- `POST /api/v1/public/auth/verify`：根据服务端保存的 challenge scope 判断是否必须提交 `walletProof + passportAssertion`；需要 Passport claims 时完成双重校验后签发应用本地 session。
- 老请求体继续走现有地址签名登录，不要求 Wallet 向 Node 领取 assertion。

```plantuml
@startuml
autonumber
actor User
participant "Community App Frontend" as App
participant "Community App Backend" as Backend
participant "Wallet Extension" as Wallet
participant "Node Passport" as Node
database "Identity DB" as DB

User -> App : 选择「钱包登录」
App -> Backend : POST /api/v1/public/auth/challenge\n(address, scope)
Backend -> Backend : 创建一次性 nonce/state\n记录 app 登录会话
Backend --> App : SIWE challenge + nonce\n需要 Passport claims 时附带 appId/audience/scope/passportEndpoint
App -> Wallet : yeying_passport_assertion\n(nonce, audience, appId, scope)
Wallet -> User : 展示登录站点、请求 scope\n含 identity.email 时明确展示邮箱授权
User -> Wallet : 确认
Wallet -> Wallet : 使用当前地址签名登录意图
Wallet -> Node : POST /api/v1/public/auth/passport/assertions/wallet\n(address, message, signature, appId, audience, nonce, scope)
Node -> Node : 校验钱包签名、地址绑定、scope、audience、nonce
Node -> DB : 查 address -> subject\n读取 verified email / 写审计
Node --> Wallet : passportAssertion(JWS/JWT/UCAN)\n含 sub、walletAddress、scope、nonce、aud、exp\n含 email claims（仅已授权且已验证）
Wallet --> App : address + walletProof + passportAssertion
App -> Backend : POST /api/v1/public/auth/verify\n(address, walletProof, passportAssertion)
Backend -> Node : 可选：获取 JWKS 或 introspect assertion\n离线验签时不必每次请求 Node
Backend -> Backend : subjectId -> localUserId\n检查禁用状态和业务权限
Backend --> App : 应用本地 session
@enduml
```

钱包登录时，DApp 后端必须同时校验：

1. `walletProof` 的地址签名有效，nonce 与当前登录会话一致，domain/audience 指向当前应用。
2. `passportAssertion` 由 Node 签发，未过期，`aud` 是当前应用，`nonce` 与钱包证明一致。
3. `passportAssertion.walletAddress` 与钱包签名恢复地址一致。
4. `passportAssertion.scope` 覆盖应用所需 scope；未请求或未授权的字段不得使用。
5. 当业务要求邮箱时，必须存在 `emailVerified: true`，并且邮箱来自 Passport assertion，不来自前端表单或钱包自声明。

Wallet 签名消息可以包含登录意图、`appId`、`audience`、`nonce` 和请求的 scopes，但 `subjectId`、`email`、`emailVerified` 不能只靠 Wallet 自声明。它们必须由 Node Passport assertion 提供。

#### 4.3.1 Assertion 声明格式

Passport assertion 可以采用 JWS/JWT、UCAN 或等价的 Node 签名结构。最低字段如下：

```json
{
  "iss": "https://node.yeying.pub",
  "sub": "sub_xxxx",
  "aud": "https://app.example.com",
  "appId": "community-app",
  "nonce": "login_nonce_from_app_backend",
  "authMethod": "wallet",
...
- `identity.basic`：返回 `sub` / `subjectId`。
- `identity.wallet`：返回当前绑定的钱包地址。
- `identity.email`：只有在 subject 邮箱已验证时返回 `email`、`emailVerified: true`、`emailVerifiedAt`；未验证时返回 `emailVerified: false` 或明确错误，由应用决定是否继续。
- 未请求的 scope 不返回对应字段，即使 Node 已保存该字段。

### 4.4 原有 Wallet 登录兼容路径（仅地址证明）
...
该路径只能证明钱包地址，默认不能获得已验证邮箱。迁移期应保留它用于仅需地址的 DApp；需要社区身份或邮箱的 DApp 再升级到 4.3 的 Passport-backed Wallet Login。若地址尚未绑定到社区主体，Node 可以引导用户完成新账户创建及通行证绑定；不得仅因某地址曾访问过某 DApp 就把它绑定给当前会话。
...
User -> App : Passkey 或 Passport-backed Wallet 登录
...
| `POST /api/v1/public/auth/passport/assertions/wallet` | Wallet | 使用钱包签名向 Node 领取 Passport assertion |
| `POST /api/v1/public/auth/passport/assertions/introspect` | Community App 后端 | 可选：在线校验 assertion 签名、过期时间和 scope |
...
1. Node 实现 Passport subject、钱包绑定、邮箱验证、WebAuthn 注册/认证、credential 模型、challenge 存储和 Node session。
2. Wallet 完成社区身份绑定、邮箱验证码确认和解绑。
3. 支持多个 discoverable passkey、查看最近使用时间、撤销和全部会话退出。
4. 完成真实 Chrome、Safari、Windows Hello、Android/iOS 的端到端测试。

### 第二阶段：跨应用身份分发

1. Chat、Project 等统一接入 Node authorization code + PKCE，用 `subjectId` 映射本地用户。
2. 实现 Passport-backed Wallet Login：Wallet 向 Node 领取 assertion，DApp 后端验证 assertion 并读取已授权 claims。
3. 明确各应用 `returnTo` allowlist、`audience`、`appId`、nonce、scope、cookie 域与 CSRF 策略。
4. 保持 UCAN/scoped credential 与 Node 登录会话分离。
