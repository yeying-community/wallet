...
> 本文定义 Wallet 云端密钥托管功能的可重复验收步骤。托管对象是钱包密钥和 Wallet Identity 全部材料的密码加密密文，不包含 MPC 分片或普通数据备份。新设备恢复必须通过 Node 身份授权兑换接口取得 `custodyRecovery` token。
...
- Wallet Identity DID controller/recovery 私钥、身份文档和身份凭证的明文；这些材料必须只出现在 Wallet 密码加密的 `ciphertext` 中；
- 用户名、邮箱、头像和 JWT-VC；
...
- 每个私钥导入钱包对应一条托管记录，记录中包含该钱包的唯一账户私钥；
...
- 页面显示“共 N 个云端托管钱包”，`N` 必须等于服务返回的托管记录数量；
- 每条记录显示 `metadata.walletName`、`metadata.accountCount` 和恢复授权对应的 Wallet Identity DID，不显示内部 `walletId`；
- 授权 DID 与服务响应不一致时必须拒绝继续。
...
3. 输入创建托管记录时使用的原钱包密码。
4. Wallet 请求对应恢复密文。
5. Wallet 在本地解密并执行版本、类型、全部账户地址、账户索引和 HD 派生校验。
6. 校验成功后导入钱包及全部账户，并清除恢复 token、PKCE 状态和密码输入框。

预期：

- 恢复成功后显示“钱包恢复成功”；
- 钱包名称、账户数量和新格式记录中的账户名称与托管前一致；
- 旧 v2 记录未保存账户名称时，主账户使用记录中的钱包名称，子账户明确显示默认名称“账户 N”；
- 新设备产生新的本地 `walletId`，但恢复地址保持一致；
- 本地导入后的钱包使用恢复输入的密码；
- 设备 B 不恢复设备 A 的站点授权、UCAN 会话或 MPC 分片；必须从密文恢复身份 controller/recovery 加密私钥及同一 DID；
- 恢复失败不得创建钱包或账户。
...
1. 设备 B 先创建或导入一个与设备 A 不同的钱包，确认该钱包可以正常使用。
2. 在设备 B 打开“账户管理”，从右上角菜单点击“云端恢复钱包”。
3. 完成与欢迎页相同的通行证授权，选择设备 A 的托管记录并输入设备 A 创建托管记录时使用的钱包密码。
4. 等待恢复完成并返回钱包首页。

预期：

- 设置页入口与欢迎页入口使用同一个恢复授权流程，不要求先重置设备 B；
- 设备 B 原有钱包、账户、授权网站和本地设置保持不变；
- 恢复的钱包以追加或同地址复用方式写入，不能覆盖其他钱包；
- 恢复完成后自动选中恢复的钱包，并恢复设备 A 的同一 Wallet Identity DID、身份控制密钥和已验证凭证；
- 后续钱包身份 Presentation 使用恢复后的 DID 直接生成，不要求重新验证用户名、邮箱或头像；
- 返回或取消恢复时回到发起恢复的页面，临时恢复授权失效后必须清理。
...
User -> Wallet : 确认钱包身份 DID 并输入托管钱包密码
