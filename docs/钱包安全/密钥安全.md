...
加密钱包备份文件是用户主动生成的离线迁移文件，格式为 `yeying-wallet-accounts` v1。它和云端密钥托管是两个独立入口，都会恢复钱包密钥和同一 Wallet Identity 控制权；区别是文件恢复不依赖 Node，云端恢复需要通行证授权。
...
payload.identityEndpoint = 当前身份服务地址（非敏感配置）
```
...
`identityEndpoint` 只记录身份服务的 HTTP(S) 地址，不是凭证或密钥。导入成功后 Wallet 会恢复这个地址，后续通行证和 TOTP 状态查询会访问同一个身份服务。旧版本文件没有该字段时，继续使用设备当前配置或默认身份服务地址。
...
6. 如果文件包含 `identityEndpoint`，Wallet 同时恢复身份服务地址；该地址必须指向原来保存通行证和 TOTP 注册记录的 Node 实例。
...
身份服务地址的恢复只恢复查询目标，不迁移 Node 端认证器记录。通行证和 TOTP 注册记录仍由 Node 按 DID 保存；如果导入到另一套 Node 数据库，状态会按另一套数据库的记录显示为未启用，需要使用原 Node 地址或在目标 Node 重新注册。
...
- 助记词、私钥或钱包密码；
- Wallet Identity DID controller/recovery 私钥、DID 文档、用户名、邮箱、头像和 JWT-VC 的明文；这些材料只存在于 Wallet 本地加密的 `ciphertext` 中；
...
1. V2 托管 payload 将 `wallet_identities` 的公开身份记录和 `encryptedKeyMaterial` 放入钱包密码加密的 `ciphertext`；Node 只保存密文。
2. `custody.recovery` 只授权读取密文，Wallet 必须在本地使用原钱包密码解密并校验身份材料，Node 和通行证服务都不能取得身份私钥。
3. Passkey 或 Node 授权证明用户有恢复资格，但不替代原钱包密码；没有原密码不能解密托管密文。
4. `createWalletIdentity()` 使用独立随机的 Ed25519 密钥生成 `wid_` 和 DID，DID 不由 HD 助记词或 EVM 地址确定性派生；因此恢复必须保留密文中的原身份记录，不能重新计算或新建 DID。

因此云端托管恢复的结果是：

- 恢复同一钱包地址和钱包账户控制权；
- 恢复同一 Wallet Identity DID 的本地签名控制权；
...
| 新设备恢复结果 | 解密并校验原 `walletIdentityId`、controller/recovery 密钥和 DID 文档后，恢复原 DID | 当前 V2 已实现 |
| Node 的职责 | 只签发短期 `custody.recovery` 读取授权，不接触明文身份材料 | 当前 V2 已实现 |
| 用户承诺 | “可在线恢复完整钱包密钥和 Wallet Identity 控制权” | 当前 V2 已实现 |
...
| 在线完整恢复 | 云端密钥托管 | 钱包密钥 + Wallet Identity | 通行证授权 + 原钱包密码 | 需要 Node/托管服务 |
| 普通数据同步 | 数据同步 | 账户名称、联系人、网络等白名单数据 | 同步授权和同步密钥 | 需要 WebDAV |

推荐产品流程：

1. 创建钱包后引导用户先生成加密钱包备份文件。
2. 用户可以单独开启云端密钥托管，并明确提示“恢复钱包密钥和同一 Wallet Identity”。
