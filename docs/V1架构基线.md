...
| 加密 | `yeying_encrypt` / `yeying_decrypt` / `yeying_getCipherSuites` | `operations/crypto-service.handleYeying*` | 是(encrypt/decrypt) / 否(getCipherSuites) | 需站点已授权 + 静默执行(同 UCAN) |
...
| R18 | DApp-facing 加密/解密方法（`yeying_encrypt/decrypt`）静默执行（站点授权即信任） | 已授权站点被 XSS 即可在用户不知情下调用加解密；但站点 XSS 本来就危险，UCAN 沿用同模型 | **接受风险** + 文档说明：详见 §4 EIP-1193 表加密行；web3-bs `docs/加解密服务.md` 写明「不应用来保护私钥/助记词」 |
