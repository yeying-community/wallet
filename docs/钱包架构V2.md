...
> 状态：已完成。Node 已实现 Passport subject、钱包绑定与安全解绑、Passkey 注册/登录，以及 authorization code + PKCE 接口；Wallet 已完成 SIWE 绑定、解绑和状态查询；Project 已接入登录会话、二维码授权、回调换码与本地账号映射，并建立 PKCE 契约测试。`Project -> Node -> Passkey -> Project` 已进入持续 E2E 验收。
...
> 应用接入：Project 已按 Node 契约生成服务端 PKCE verifier、创建授权请求、展示 `verifyUrl`、接收一次性 code，并在服务端换取身份结果。Project 不向浏览器返回 verifier；回调必须命中仍有效的本地 `state` 会话。真实 Node 与浏览器环境已持续验证该路径。
