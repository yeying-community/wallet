...
>
> 目标读者：钱包、协调器和安全架构维护者。生产化门槛见 [钱包架构决策](../钱包架构/钱包架构决策.md)。
>
...
> 决策状态：分两层
>
> - 2026-08-08 决策(本文档初版):保持实验性,不承载生产资产
> - 2026-09-12 决策([`MPC生产化评审决策.md`](./MPC生产化评审决策.md)):取消前置禁令,进入生产评审阶段。**§2 9 项准入条件仍未全部满足,§16.1 描述的 dev-verification build 仍是当前仓库的实际构建**

本文档保留 2026-08-08 决策的完整历史记录,并标注 2026-09-12 决策对其的修订。读者应同时阅读 [`MPC生产化评审决策.md`](./MPC生产化评审决策.md) 以理解当前状态。

### 2026-09-12 起生效

- 解除"不得接收生产资产"、"不得进入生产交易签名路径"前置禁令。
- dev-trusted 逃生口 `_completeKeyShareFromDevTrustedAuxInfo` 加 `productionSafe !== true` 独立 gate(见 §3 与 `mpc-service.js:_isDevFallbackAllowed`),确保 productionSafe build 下绝对不放行。
- §2 第 75-77 行发布约束中"默认发布配置不得把实验性 MPC 作为推荐钱包类型"在 UI 默认隐藏的前提下仍生效。

### 2026-08-08 决策原文(保留为历史)
...
- ~~引导用户向实验性 MPC Wallet 转入生产资产~~(2026-09-12 起放宽)
- 对外宣称当前实现达到生产级门限钱包安全。
...
### 16.3 2026-09-12 补充观测

承接 commit `1a55ac6` (并行素数生成) 与 `8220e11` (aux-info 收敛：deadline + 代际重试 + 看门狗)：

- 当前 `js/background/wasm/cggmp24/mpc_cggmp24_spike.wasm` 仍为 dev-verification build(`securityProfile: 'dev-verification'` + `productionSafe: false`)，该 build 下 SW 端 `_completeKeyShareFromDevTrustedAuxInfo` 路径仍然存在，但自 2026-09-12 起新增独立 gate `_isDevFallbackAllowed`(参见 `mpc-service.js:_isDevFallbackAllowed`)显式拒绝 productionSafe build 上的 dev 逃生口。
- 真机 Chrome P 核 caffeinate 3 轮交错验证(commit `1a55ac6`):over 6→4 中位 **34.9s**、最坏 **66.6s**;naive parallel-4 中位 **118.2s**、最坏 **130.8s**;serial baseline **120.7s**。**该数字仅适用于 dev-verification build,productionSafe build 未编译过、未真机验证过**。
- 收敛(commit `8220e11`):10 min deadline + 3 代代际重试 + 1 min 看门狗 alarm;持久化 `auxInfoStartedAt` + `auxInfoGeneration` 作为 deadline 锚点 + requestId 命名空间。**崩溃恢复仅通过代码审计得出"恢复路径不影响正确性"的结论,真机 SW-kill → 看门狗 → 代际重启 端到端未做**(详见 §17 验收用例与 [`MPC生产化评审决策.md`](./MPC生产化评审决策.md) §5)。
- 失败可观测性:fan-out 失败时 worker enrich error message(`<原始原因> | fanout={poolSize, take, wallMs, successCount, failureCount, error}`)自动进 `wire-aux-info-start-skipped` 审计 + `aux-info:start-failed` debug log;成功路径 worker 打 `cggmp24:aux-prime-fanout` debug log。
