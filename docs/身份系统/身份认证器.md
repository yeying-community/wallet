...
Node 的注册、列表、撤销和无插件授权接口分别使用 registration request/confirm、list、revoke 以及 authorize request/challenge/approve/exchange。所有接口都必须绑定 DID，并校验真实 origin、RP ID 和 challenge。
...
TOTP 生命周期由 setup、confirm、verify、status 和 revoke 组成：setup 创建 pending secret，confirm 使用首次验证码激活，verify 只接受 active 认证器，revoke 立即失效。setup/revoke 必须使用身份文档的 manage proof；secret 仅在 setup 响应中返回 Wallet，Node 持久化密文。
