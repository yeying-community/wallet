...
- 头像是 Node issuer 签发的 `AvatarCredential`，与同一次资料验证事务绑定。
...
component "Credentials\nUsernameCredential / EmailCredential / AvatarCredential" as Credentials
...
5. 用户填写用户名和邮箱。
6. Node 发送邮箱验证码。
7. 用户输入验证码后，Node 签发 `UsernameCredential`、`EmailCredential`，并在用户填写头像 URI 时签发 `AvatarCredential`。
...
Node --> Wallet : UsernameCredential + EmailCredential + optional AvatarCredential
...
DApp 通过 web3-bs 先请求 Wallet 连接账户并授权钱包身份 scope：
...
Wallet 在连接请求页的“此网站请求以下权限”中展示账户权限和身份权限，不再弹出单独的身份资料授权页。确认连接后，DApp 再请求 Wallet 出示钱包身份 presentation：
...
头像凭证结构相同，`type` 改为 `AvatarCredential`，`credentialSubject` 至少包含：

```json
{
  "id": "did:yeying:wid_example",
  "avatarUri": "https://avatar.example/person.png",
  "avatarVerifiedAt": "2026-08-13T00:00:00Z",
  "avatarVerificationMethod": "wallet-profile-v1"
}
```

邮箱凭证只能在 `identity.email` scope 被请求，头像凭证只能在 `identity.avatar` scope 被请求；凭证必须仍在 `nbf`/`exp` 有效窗口内，且经用户确认时出示。Wallet 生成 presentation 前必须先检查本地凭证有效期；已过期、临近过期或尚未生效的凭证不得进入 `credentials[]`。如果 DApp 请求中提供了 `issuerEndpoint`，Wallet 会先用当前 identity controller 对 Node 一次性 challenge 签名，自动换取新的同类型 JWT-VC，再生成 presentation。
...
- `UsernameCredential`、`EmailCredential` 与 `AvatarCredential` 默认有效期为 24 小时，最长不得超过 7 天。
...
- 源凭证不存在、已撤销、身份 controller proof 无效或 issuer endpoint 不可用时，Wallet 不得降级出示过期凭证，应提示用户重新完成资料验证。
...
- UsernameCredential、EmailCredential、AvatarCredential 的签名、subject、scope、过期、撤销。
