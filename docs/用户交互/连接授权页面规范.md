...
participant DApp
participant Wallet
DApp -> Wallet: wallet_requestPermissions(identity scopes)
Wallet --> DApp: 连接请求页面
Wallet -> Wallet: 用户确认身份权限
DApp -> Wallet: wallet_identity_presentation
Wallet --> DApp: 解锁钱包页面（需要时）
Wallet -> Wallet: 使用身份私钥签名 Presentation
Wallet --> DApp: 返回 Presentation
DApp -> DApp: 服务端验证并建立会话
@enduml
```

连接请求页面只展示实际请求的权限：`identity.basic`、`identity.wallet`、`identity.username`、`identity.email`、`identity.avatar`。未请求 `eth_accounts` 时，不显示账户地址；余额和链下签名不属于身份登录权限。

页面按语义压缩为最多三项：钱包身份、钱包账户关联证明、已验证资料。已验证资料一项逐字列出本次请求的用户名、邮箱和头像。该分组只用于界面展示，Provider 请求和授权记录仍分别保存每个 scope。
...
participant Node
participant Browser
DApp -> Node: 创建 authorization code + PKCE 会话
Node --> DApp: 返回 verifyUrl
DApp -> Node: 打开通行证登录页面
Node -> Browser: WebAuthn get()
Browser --> Node: 通行证选择和生物识别
Node --> DApp: authorization code
DApp -> Node: 使用 code + PKCE exchange
Node --> DApp: DID 和授权凭证
DApp -> DApp: 建立会话
@enduml
```

通行证登录不显示 Wallet 连接请求、钱包解锁或身份资料授权页面。设备选择和指纹验证属于浏览器/操作系统 WebAuthn 界面。
